...
from sam2.modeling.sam2_new_base import SAM2NewBase
from sam2.utils.misc import concat_points, fill_holes_in_mask_scores, load_video_frames


class SAM2VideoPredictorNew(SAM2NewBase):
    ...
    @classmethod
    def from_pretrained(cls, model_id: str, **kwargs) -> "SAM2VideoPredictorNew":
        """
        ...
        Returns:
          (SAM2VideoPredictorNew): The loaded model.
        """
    ...
    def _run_single_frame_inference(
        ...
        pred_masks_high_res = current_out.get("pred_masks_high_res", None)
        if pred_masks_high_res is not None:
            pred_masks_high_res = pred_masks_high_res.to(storage_device, non_blocking=True)

        ious = current_out.get("ious", None)
        if ious is not None:
            ious = ious.to(storage_device, non_blocking=True)
...
class SAM2VideoPredictorVOSNew(SAM2VideoPredictorNew):
    ...
    def forward_image(self, img_batch: torch.Tensor):
        """
        Identical to the corresponding method in the parent (SAM2VideoPredictorNew), but
