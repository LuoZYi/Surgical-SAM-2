#!/bin/bash
#SBATCH --job-name=vos_2017_rule_recent
#SBATCH --output=shell/local/endovis17_new/logs/vos_2017_rule_recent_%j.out
#SBATCH --error=shell/local/endovis17_new/logs/vos_2017_rule_recent_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2017_rule_recent.out"
LOG_ERR="${LOG_DIR}/vos_2017_rule_recent.err"
...
mkdir -p "${RESULTS_ROOT}/endovis17_new/vos_2017_rule_recent"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/endovis17_new/vos_2017_rule_recent" \
  ...
  --recent_memory_min_keep 1 \
  --recent_memory_max_keep 2 \
  --recent_similarity_threshold 0.985 \
  --recent_stability_threshold 0.72 \
  --recent_confidence_threshold 0.60
