#!/bin/bash
#SBATCH --job-name=vos_2017_state
#SBATCH --output=shell/local/endovis17_new/logs/vos_2017_state_%j.out
#SBATCH --error=shell/local/endovis17_new/logs/vos_2017_state_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2017_state.out"
LOG_ERR="${LOG_DIR}/vos_2017_state.err"
...
mkdir -p "${RESULTS_ROOT}/endovis17_new/vos_2017_state"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/endovis17_new/vos_2017_state" \
