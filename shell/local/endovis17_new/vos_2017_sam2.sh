#!/bin/bash
#SBATCH --job-name=vos_2017_sam2
#SBATCH --output=shell/local/endovis17_new/logs/vos_2017_sam2_%j.out
#SBATCH --error=shell/local/endovis17_new/logs/vos_2017_sam2_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2017_sam2.out"
LOG_ERR="${LOG_DIR}/vos_2017_sam2.err"
...
mkdir -p "${RESULTS_ROOT}/endovis17_new/vos_2017_sam2"

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  --sam2_cfg configs/sam2.1/sam2.1_hiera_s.yaml \
  --sam2_checkpoint ./checkpoints/sam2.1_hiera_small.pt \
  --output_mask_dir "${RESULTS_ROOT}/endovis17_new/vos_2017_sam2" \
