...
#SBATCH --output=shell/local/endovis17_new/logs/vos_2017_hydra_efp_adaptive_controller_%j.out
#SBATCH --error=shell/local/endovis17_new/logs/vos_2017_hydra_efp_adaptive_controller_%j.err
...
mkdir -p "${RESULTS_ROOT}/endovis17_new/vos_2017_efp_adaptive_controller"

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/hydra_infer.py \
  dataset=endovis2017 \
  memory_policy=efp_adaptive_controller \
  output_mask_dir="${RESULTS_ROOT}/endovis17_new/vos_2017_efp_adaptive_controller" \
