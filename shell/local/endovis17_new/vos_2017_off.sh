...
#SBATCH --output=shell/local/endovis17_new/logs/vos_2017_off_%j.out
#SBATCH --error=shell/local/endovis17_new/logs/vos_2017_off_%j.err
...
mkdir -p "${RESULTS_ROOT}/endovis17_new/vos_2017_off"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/endovis17_new/vos_2017_off" \
