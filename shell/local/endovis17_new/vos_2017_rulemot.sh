#!/bin/bash
#SBATCH --job-name=vos_2017_rulemot
#SBATCH --output=shell/local/endovis17_new/logs/vos_2017_rulemot_%j.out
#SBATCH --error=shell/local/endovis17_new/logs/vos_2017_rulemot_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2017_rulemot.out"
LOG_ERR="${LOG_DIR}/vos_2017_rulemot.err"
...
mkdir -p "${RESULTS_ROOT}/endovis17_new/vos_2017_rulemot"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/endovis17_new/vos_2017_rulemot" \
  ...
  --memory_score_mode cosine_motion \
