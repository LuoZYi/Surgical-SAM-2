#!/bin/bash
#SBATCH --job-name=vos_2017_rulemotgeo
#SBATCH --output=shell/local/endovis17_new/logs/vos_2017_rulemotgeo_%j.out
#SBATCH --error=shell/local/endovis17_new/logs/vos_2017_rulemotgeo_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2017_rulemotgeo.out"
LOG_ERR="${LOG_DIR}/vos_2017_rulemotgeo.err"
...
mkdir -p "${RESULTS_ROOT}/endovis17_new/vos_2017_rulemotgeo"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/endovis17_new/vos_2017_rulemotgeo" \
