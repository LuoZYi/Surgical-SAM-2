#!/bin/bash
#SBATCH --job-name=vos_2018_hydra_efp_adaptive_controller
#SBATCH --output=shell/local/endovis2018/logs/vos_2018_hydra_efp_adaptive_controller_%j.out
#SBATCH --error=shell/local/endovis2018/logs/vos_2018_hydra_efp_adaptive_controller_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2018_hydra_efp_adaptive_controller.out"
LOG_ERR="${LOG_DIR}/vos_2018_hydra_efp_adaptive_controller.err"
...
mkdir -p "${RESULTS_ROOT}/vos_2018_efp_adaptive_controller"

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/hydra_infer.py \
  dataset=endovis2018 \
  memory_policy=efp_adaptive_controller \
  output_mask_dir="${RESULTS_ROOT}/vos_2018_efp_adaptive_controller" \
