#!/bin/bash
#SBATCH --job-name=vos_2018_hydra_efp_learned_skip_write_v1
#SBATCH --output=shell/local/endovis2018/logs/vos_2018_hydra_efp_learned_skip_write_v1_%j.out
#SBATCH --error=shell/local/endovis2018/logs/vos_2018_hydra_efp_learned_skip_write_v1_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2018_hydra_efp_learned_skip_write_v1.out"
LOG_ERR="${LOG_DIR}/vos_2018_hydra_efp_learned_skip_write_v1.err"
...
mkdir -p "${RESULTS_ROOT}/vos_2018_efp_learned_skip_write_v1"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/hydra_infer.py \
  ...
  output_mask_dir="${RESULTS_ROOT}/vos_2018_efp_learned_skip_write_v1" \
