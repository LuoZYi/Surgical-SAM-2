...
#SBATCH --output=shell/local/endovis2018/logs/vos_2018_rulemotgeo_%j.out
#SBATCH --error=shell/local/endovis2018/logs/vos_2018_rulemotgeo_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2018_rulemotgeo.out"
LOG_ERR="${LOG_DIR}/vos_2018_rulemotgeo.err"
...
mkdir -p "${RESULTS_ROOT}/vos_2018_rulemotgeo"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/vos_2018_rulemotgeo" \
