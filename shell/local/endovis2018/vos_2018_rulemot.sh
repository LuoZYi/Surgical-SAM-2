...
#SBATCH --output=shell/local/endovis2018/logs/vos_2018_rulemot_%j.out
#SBATCH --error=shell/local/endovis2018/logs/vos_2018_rulemot_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2018_rulemot.out"
LOG_ERR="${LOG_DIR}/vos_2018_rulemot.err"
...
mkdir -p "${RESULTS_ROOT}/vos_2018_rulemot"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/vos_2018_rulemot" \
