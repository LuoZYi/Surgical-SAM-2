...
#SBATCH --output=shell/local/endovis2017/logs/vos_2017_rule_recent_%j.out
#SBATCH --error=shell/local/endovis2017/logs/vos_2017_rule_recent_%j.err
...
mkdir -p "${RESULTS_ROOT}/vos_2017_rule_recent"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/vos_2017_rule_recent" \
