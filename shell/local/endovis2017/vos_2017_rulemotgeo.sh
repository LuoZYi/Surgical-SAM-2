...
#SBATCH --output=shell/local/endovis2017/logs/vos_2017_rulemotgeo_%j.out
#SBATCH --error=shell/local/endovis2017/logs/vos_2017_rulemotgeo_%j.err
...
mkdir -p "${RESULTS_ROOT}/vos_2017_rulemotgeo"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/vos_2017_rulemotgeo" \
