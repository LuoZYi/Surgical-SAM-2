...
#SBATCH --output=shell/local/endovis2017/logs/vos_2017_hydra_efp_learned_skip_write_v1_%j.out
#SBATCH --error=shell/local/endovis2017/logs/vos_2017_hydra_efp_learned_skip_write_v1_%j.err
...
mkdir -p "${RESULTS_ROOT}/vos_2017_efp_learned_skip_write_v1"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/hydra_infer.py \
  ...
  output_mask_dir="${RESULTS_ROOT}/vos_2017_efp_learned_skip_write_v1" \
