...
#SBATCH --output=shell/local/endovis2017/logs/vos_2017_hydra_efp_adaptive_skip_write_v2_%j.out
#SBATCH --error=shell/local/endovis2017/logs/vos_2017_hydra_efp_adaptive_skip_write_v2_%j.err
...
mkdir -p "${RESULTS_ROOT}/vos_2017_efp_adaptive_skip_write_v2"

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/hydra_infer.py \
  dataset=endovis2017 \
  memory_policy=efp_adaptive_skip_write_v2 \
  output_mask_dir="${RESULTS_ROOT}/vos_2017_efp_adaptive_skip_write_v2" \
