...
#SBATCH --output=shell/local/endovis2017/logs/vos_2017_off_%j.out
#SBATCH --error=shell/local/endovis2017/logs/vos_2017_off_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2017_off.out"
LOG_ERR="${LOG_DIR}/vos_2017_off.err"
...
mkdir -p "${RESULTS_ROOT}/vos_2017_off"

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference_vanilla_sam2.py \
  --sam2_cfg configs/sam2.1/sam2.1_hiera_s.yaml \
  --sam2_checkpoint ./checkpoints/sam2.1_hiera_small.pt \
  --output_mask_dir "${RESULTS_ROOT}/vos_2017_off_no_finetune" \
