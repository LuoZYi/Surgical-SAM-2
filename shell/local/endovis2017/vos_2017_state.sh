...
#SBATCH --output=shell/local/endovis2017/logs/vos_2017_stateaware_%j.out
#SBATCH --error=shell/local/endovis2017/logs/vos_2017_stateaware_%j.err
...
LOG_OUT="${LOG_DIR}/vos_2017_stateaware.out"
LOG_ERR="${LOG_DIR}/vos_2017_stateaware.err"
...
mkdir -p "${RESULTS_ROOT}/vos_2017_stateaware"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV_NAME}" python tools/vos_inference.py \
  ...
  --output_mask_dir "${RESULTS_ROOT}/vos_2017_stateaware" \
